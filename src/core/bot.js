...
const {
  generateMaxPriorityFeeChart,
  generateOtherFeesChart,
} = require('../charts/chartGeneratorGist')
...
bot.on('callback_query', async (callbackQuery) => {
  ...
  switch (data) {
    ...
    default:
      bot.sendMessage(
        chatId,
        'Неизвестная команда, пожалуйста, выберите одну из доступных опций.',
      )
  }
})
...
async function fetchAndSendBaseFeeHistory(chatId) {
  try {
    const baseFeeHistory = await fetchBaseFeeHistory(
...
async function fetchAndSendBaseFeePercentile(chatId) {
  const percentileData = await fetchBaseFeePercentile(
    process.env.INFURA_API_KEY,
    process.env.INFURA_API_KEY_SECRET,
  )
  if (percentileData) {
...
async function fetchAndSendBusyThreshold(chatId) {
  try {
    ...
    const busyThreshold = await fetchBusyThreshold(
      process.env.INFURA_API_KEY,
      process.env.INFURA_API_KEY_SECRET,
    )
...
async function fetchAndSendGasPrices(chatId) {
  const gasFeesData = await fetchSuggestedGasFees(
    process.env.INFURA_API_KEY,
    process.env.INFURA_API_KEY_SECRET,
  )
  if (gasFeesData) {
        ...
        [
          {
            ...
            callback_data: 'fetch_base_fee_chart',
          },
        ],
        [
          {
            text: 'Получите процентиль базовой комиссии',
            callback_data: 'fetch_percentile',
          },
        ],
        [
          {
            text: 'Получите порог занятости',
            callback_data: 'fetch_busy_threshold',
          },
        ],
        [
          {
            text: 'Получить цены на газ EIP-1559',
            callback_data: 'fetch_gas_prices',
          },
        ],
